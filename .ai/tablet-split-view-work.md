...
- 1320x2856 手机模拟器验证通过：
  - 主页保持单栏 `Tabs`，未误用平板 `SideBarContainer`。
  - 新建笔记通过 `router.pushUrl` 进入独立 `NoteEditPage`，保存后返回首页并刷新列表。
  - 点击已有笔记可再次进入编辑页，系统返回键可正常回到首页。
  - 新建待办通过 `router.pushUrl` 进入独立 `TodoDetailPage`，保存后返回待办列表并刷新。
  - 点击已有待办可再次进入详情页。
  - 全流程日志中无 `TypeError`、`JS_ERROR` 或运行时崩溃。
